...
int SceneReader::initializeScene(std::string fileName) {
  ...
  float d = 2.0f;
  viewHeight = 2.0f * d * tan(0.5 * vfov * degToRad);
  /*float d = viewHeight / (2.0f * tan(0.5f * vfov * degToRad));*/
...
Vector3 SceneReader::imageToView(int row, int col) {
  Vector3 deltaH =
      Vector3::multiply(1.0f / (width - 1.0f), Vector3::subtract(ur, ul));
  Vector3 deltaV =
      Vector3::multiply(1.0f / (height - 1.0f), Vector3::subtract(ll, ul));

  Vector3 result = Vector3::add(
      ul, Vector3::add(Vector3::multiply(static_cast<float>(row), deltaH),
                       Vector3::multiply(static_cast<float>(col), deltaV)));
  return result;
}
...
Color SceneReader::traceRay(const Ray &ray) const {
  ...
  for (const Sphere &sphere : spheres) {
    ...
    float originMinusCenterX =
        ray.getOrigin().getX() - sphere.getCenter().getX();
    float originMinusCenterY =
        ray.getOrigin().getY() - sphere.getCenter().getY();
    float originMinusCenterZ =
        ray.getOrigin().getZ() - sphere.getCenter().getZ();

    float a = dirX * dirX + dirY * dirY + dirZ * dirZ;
    float b =
        2.0f * (dirX * (originMinusCenterX) + dirY * (originMinusCenterY) +
    ...
    float t2 = -b - sqrt(b * b - 4.0f * c) / 2.0f;
